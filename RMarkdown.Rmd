...
```{r}
#creating a df specifying dummies

dummy_df <- merged_df %>% 
  mutate(
    years2011_2015 = if_else(between(startYear, 2011, 2015), 1, 0),
    years2016_2020 = if_else(between(startYear, 2016, 2020), 1, 0)
  )

dummy_df <- dummy_df %>%
  mutate(
    comedy    = if_else(str_detect(genres, "Comedy"), 1L, 0L),
